...
db {Source:\"couchdb\", Default-Type:\"docker:latest\", Version-Info: \"Tested with 1.7 & 2\"}
  -t db:local #reuse a local, running CouchDB installation, does not start/stop this CouchDB
  -t db:docker:[TAG] #start docker, default tag latest, uses image https://hub.docker.com/_/couchdb
...
if [ "$SKIP_HASH_CHECK" != "YES" ]; then
	if which md5 1>/dev/null; then
		declare SELF_HASH_MD5="f1b1adc154903c8ead4df942b014e0f1"
...
if [ "$START_ENGINE" = "YES" ]; then
	...
	if [ "$TYPE_SOURCE_ENGINE" == "local" ]; then
		if [ ! -f ".enginePid" ]; then
			...
			export CYC_ENGINE_CONTAINER="localrun/cyc-engine-container"
			export OPTS="-XX:+UnlockExperimentalVMOptions -XX:+UseCGroupMemoryLimitForHeap -Dcyc.properties=/tmp/b6216549/cyc_engine.properties"
...
EOT34aabfcf
			...
			export JAVA_OPTS="$JAVA_OPTS -DJAVA_OPTS=-Dcyc.properties=/tmp/34aabfcf/cyc_web.properties"
